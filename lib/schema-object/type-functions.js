...
const getArrayRequirement = (dataPath, isRequired, child, arrays) => {
  // Determine the array item type for the type description
  const itemType = child.value.items.type;
  const arrayTypeDescription = `array of ${itemType}s`;

  const coreRequirement = {
    type: () => arrayTypeDescription,
    required: () => isRequired,
  ...
  // For primitive types, merge their attributes into the parent array requirement
  if (child.value.items.type !== 'object') {
    const { items } = child.value;

    // Add item-specific attributes based on the primitive type
    if (items.type === 'string') {
      coreRequirement.minLength = () => items.minLength || '';
      coreRequirement.maxLength = () => items.maxLength || '';
      coreRequirement.format = () => items.format || '';
      coreRequirement.pattern = () => items.pattern || '';
      coreRequirement.enum = () => (items.enum || []).join(', ');
  ...
  // No longer generate additionalRequirement for primitive types
  return {
    coreRequirement,
  };
};
