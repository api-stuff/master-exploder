...
const { getJsonPath, getRequestBodyContentTypes, getResponseContentTypes } = require('./util');
...
const explodeOpenApi = async (
  ...
  const requestBodies = getRequestBodyContentTypes(api, requestContentType);
  const responseBodies = getResponseContentTypes(api, responseContentType);
  const unreferencedSchemaObjects = [];

  // This has been scrapped as the approach is more permissive now i.e. if you find it, return it
  // otherwise return the first one you find regardless.
